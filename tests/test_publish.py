...
''')
    ...
    def test_feature_branch_cannot_publish(self):
        self.assertIn('runs from main', self.attempt(branch='feat/example'))

    def test_preexisting_staged_changes_cannot_publish(self):
        self.assertIn('existing staged changes', self.attempt(staged='1'))

    def test_unreviewed_changes_are_refused(self):
        self.assertIn('Unreviewed non-edition changes', self.attempt(dirty='template.html'))
    ...
    def test_split_outputs_are_staged_on_content_publish(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'AGENTS.md').write_text('Reviewed workflow change')
...
elif a == ['diff', '--name-only', '-z']: sys.stdout.write('AGENTS.md\\0deep-dives.html\\0deep-dives/2026-09-06.html\\0')
elif a == ['diff', '--check']: pass
elif a == ['ls-files', '--others', '--exclude-standard', '-z']: pass
...
''')
            ...
            result = subprocess.run(['bash', str(root / 'publish.sh'), '2026-09-06', 'AGENTS.md'], env=env, capture_output=True, text=True)
